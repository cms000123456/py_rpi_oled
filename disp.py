...
import ImageDraw 
from PIL import ImageFont GPIO.setmode(GPIO.BCM) GPIO.setup(23, GPIO.OUT) GPIO.setup(20, GPIO.IN) DISP_OFF = 0xAE
# Raspberry Pi pin configuration:
INFO_BTN = 20 LED = 23 RST = None
# Timer for Display timeout
disp_timer = 0 DISP_TIMEOUT = 15
# Menu Variables
menu_state = 0 # 0 = Info; 1 = Reboot; 2 = Restart menu_timer = 0 REBOOT_TIMEOUT = 5 SHUTDOWN_TIMEOUT = 10 do_reboot = 0 do_shutdown = 0
...
padding = -2 top = padding bottom = height-padding
# Move left to right keeping track of the current x position for drawing shapes.
x = 0
# Load default font.
font = ImageFont.load_default()
# Alternatively load a TTF font.  Make sure the .ttf font file is in the same directory as the python script! Some other nice fonts to try: 
# http://www.dafont.com/bitmap.php font = ImageFont.truetype('Minecraftia.ttf', 8)
GPIO.output(LED, GPIO.HIGH)
# Startup Info
draw.rectangle((0,0,width,height), outline=0, fill=0) draw.text((x, top), "--------------------", font=font, fill=255) draw.text((x, top+12), " Infoscreen 
Started ", font=font, fill=255) draw.text((x, top+24), "--------------------", font=font, fill=255) disp.image(image) disp.display() time.sleep(5) while True:
    # Draw a black filled box to clear the image.
    draw.rectangle((0,0,width,height), outline=0, fill=0)
    # Info Button pressed?
    ...
    if disp_timer > 0:
        if menu_state == 0:
            ...
            # cmd = "df -h | awk '$NF==\"/\"{printf \"Disk: %d/%dGB %s\", $3,$2,$5}'" Disk = subprocess.check_output(cmd, shell = True ) Write two lines of 
            # text.
            ...
            # draw.text((x, top+27), str(MemUsage), font=font, fill=255) draw.text((x, top+25), str(Disk), font=font, fill=255)
